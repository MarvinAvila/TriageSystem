...
export class TurnoRepository {
  ...
  async asignarMedicoYCambiarEstado(
    turnoId: number,
    medicoId: number,
    estado: TurnoEstado,
  ): Promise<void> {
  ...
  async obtenerTurnosPublicos(): Promise<any[]> {
    const query = `
        SELECT t.id as "turnoId", p.nombre as "pacienteNombre", t.prioridad, t.estado
        FROM turno t
        JOIN paciente p ON t.paciente_id = p.id
        WHERE t.estado IN ('EN_COLA', 'ATENDIENDO')
        ORDER BY t.estado ASC, t.prioridad DESC, t.ts_creado ASC
        LIMIT 10;
    `;
    const result = await readPool.query(query); // Va al nodo de lectura
    return result.rows;
  }
}
